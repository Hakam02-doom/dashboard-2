...
export async function verifiedOwner(req,client){
 const token=String(req.headers.authorization||'').replace(/^Bearer /,'');if(!token)throw Error('Sign in through Settings to access cloud monitoring.');
...
export function createRuntime(env,{local=false,directory='.local-ai'}={}){
 ...
 const collector=searchapiHandler({direct,local:true,key:env.SEARCHAPI_API_KEY,analysisKey:env.OPENAI_API_KEY,analysisBudget:Number(env.AI_ANALYSIS_BUDGET_USD||1),store});
