...
function Bars({brands,colorFor,onSelect,metric='visibility'}) {
...
function BrandDonut({brands,colorFor,onSelect,onDetails}) {
 ...
 let offset=0;
 const slices=brands.filter(b=>b.sov>0).map(b=>{const start=offset;offset+=b.sov;return {...b,start,angle:(start+b.sov/2)/100*Math.PI*2-Math.PI/2};});
 return <div className="avr-orbit-chart">
  <svg viewBox="0 0 320 320" aria-label="Share of voice by brand" role="img"><circle className="avr-orbit-track" cx="160" cy="160" r="94"/>{slices.map(b=><circle key={b.name} className="avr-orbit-segment" cx="160" cy="160" r="94" pathLength="100" fill="none" stroke={colorFor(b.name)} strokeDasharray={`${Math.max(0,b.sov-.35)} ${100-Math.max(0,b.sov-.35)}`} strokeDashoffset={-b.start} transform="rotate(-90 160 160)" onMouseEnter={()=>setHovered(b)} onMouseLeave={()=>setHovered(null)} onClick={()=>onSelect(b.name)}><title>{b.name}: {pct(b.sov)}</title></circle>)}</svg>
  ...
  {slices.map(b=><button key={b.name} className="avr-orbit-logo" style={{left:`${50+Math.cos(b.angle)*42}%`,top:`${50+Math.sin(b.angle)*42}%`}} aria-label={`${b.name}: ${pct(b.sov)} share of voice. View competitor`} onFocus={()=>setHovered(b)} onBlur={()=>setHovered(null)} onMouseEnter={()=>setHovered(b)} onMouseLeave={()=>setHovered(null)} onClick={()=>onSelect(b.name)}><BrandLogo name={b.name} domain={b.domain}/></button>)}
  {hovered&&<div className="avr-orbit-tooltip" role="status"><Brand brand={hovered}/><span>Share of voice <strong>{pct(hovered.sov)}</strong></span></div>}
 </div>;
}
function Rankings({brands,selected,onSelect,colorFor,query='',sort,setSort}) {
  ...
  return <div className="avr-rank-scroll" tabIndex={0} aria-label="Brand rankings, scroll for more columns"><table><thead><tr><th>#</th><th>Brand</th>{[['visibility','Visibility'],['sov','SOV'],['sentiment','Sentiment'],['position','Avg position']].map(([key,label])=><th key={key} aria-sort={sort.key===key?(sort.asc?'ascending':'descending'):'none'}><button onClick={()=>setSort({key,asc:sort.key===key?!sort.asc:key==='position'})}>{label}<ChevronDown size={12}/></button></th>)}</tr></thead><tbody>{ordered.map(b=><tr key={b.name} className={selected===b.name?'selected':''}><td>{brands.indexOf(b)+1}</td><td><button className="avr-brand-button" onClick={()=>onSelect(b.name)} aria-pressed={selected===b.name}><Brand brand={b} color={colorFor(b.name)}/></button></td><td>{pct(b.visibility)}</td><td>{pct(b.sov)}</td><td><span className="avr-sentiment-cell">{b.sentiment!==null&&<Smile size={14}/>} {b.sentiment===null?'—':Math.round(b.sentiment)}</span></td><td>{b.position==null?'—':`#${b.position.toFixed(1)}`}</td></tr>)}</tbody></table>{!ordered.length&&<p className="avr-no-results">No brands match your search.</p>}</div>;
}
  ...
  return <>
    ...
    <div className="avr-grid">
      <Card title="Visibility" Icon={Eye} note={visibilityView==='bars'?'Percentage of answers mentioning each brand':'Brand comparison across collected answers'} controls={<Switch label="Visibility chart type" value={visibilityView} onChange={setVisibilityView} options={[["chart",ChartBar,"Horizontal visibility comparison"],["bars",BarChart3,"Visibility by brand"]]}/>} footer={footer}>
        <Score label="Visibility score" value={pct(selectedBrand.visibility)}/>{visibilityView==='bars'?<Bars brands={brands} colorFor={colorFor} onSelect={setSelected}/>:<HorizontalComparison brands={brands} selected={selectedBrand.name} colorFor={colorFor} onSelect={setSelected}/>}
      </Card>
      <Card title="Rankings" Icon={BarChart3} note="Click a brand to view as competitor" footer={<><span>{brands.length>1?`${brands.length} comparison brands`:'No tracked competitors in these results'}</span><span title="Positive = 100, neutral = 50, negative = 0. Average across assessed answers.">Sentiment score · 0–100</span></>}><Rankings brands={brands} selected={selectedBrand.name} onSelect={setSelected} colorFor={colorFor} query={search} sort={sort} setSort={setSort}/></Card>
